namespace Aiursoft.MarkToHtml.Authorization;
    ...
    public static List<PermissionDescriptor> GetAllPermissions()
    {
        ...
        [
            ...
            new(AppPermissionNames.CanReadAllDocuments,
                ...
                localizer["Allows viewing all documents in the system, regardless of ownership or sharing settings."]),
            new(AppPermissionNames.CanEditAnyDocument,
                localizer["Edit Any Document"],
                localizer["Allows editing any document in the system, even if not owned by the user."]),
            new(AppPermissionNames.CanDeleteAnyDocument,
                localizer["Delete Any Document"],
                localizer["Allows deleting any document in the system, even if not owned by the user."]),
            new(AppPermissionNames.CanManageAnyShare,
                localizer["Manage Any Share"],
                localizer["Allows managing document sharing permissions for any document, even if not owned by the user."])
        ];
